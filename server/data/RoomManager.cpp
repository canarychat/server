...
HttpParamJSON RoomManager::getRoomList(int user_id) {
    ...
    try {
        ...
        return {HTTPStatus::HTTP_INTERNAL_SERVER_ERROR, result};
    }
...
HttpParamJSON RoomManager::createRoom(int owner_id, std::string room_name, std::string room_description) {
    ...
    try {
        ...
        if (p_user) {
            ...
            room->name(room_name).description(room_description).owner_idID(owner_id);
            room->create(p_context);

            ChatRoomDB::UserRoomRelation::Ptr userRoomRelation = new ChatRoomDB::UserRoomRelation;
            userRoomRelation->user_id(p_user).room_id(room);
            userRoomRelation->create(p_context);

            result->set("code", static_cast <int>(state_code::SUCCESS));
            result->set("msg", "创建房间成功");
            return {HTTPStatus::HTTP_CREATED, result};
        } else {
            result->set("code", static_cast <int>(state_code::CHATROOM_CLIENT_ERROR));
            result->set("msg", "用户不存在");
            return {HTTPStatus::HTTP_NOT_FOUND, result};
        }
    } catch (Poco::Exception &e    ) {
        poco_error(Application::instance().logger(), e.displayText());
        result->set("code", static_cast<int>(state_code::CHATROOM_SERVER_ERROR));
...
HttpParamJSON RoomManager::getRoomMemberList(int room_id, int user_id) {
    Poco::JSON::Object::Ptr result = new Poco::JSON::Object;
    ...
    try {
        ...
        if (auto status = std::get<0>(check_exit(result, p_context, room_id, user_id));
        ...
        if (checkUser.execute() == 0) {
            ...
            return {HTTPStatus::HTTP_FORBIDDEN, result};
        }
...
HttpParamJSON RoomManager::joinRoom(int room_id, int user_id) {
    Poco::JSON::Object::Ptr result = new Poco::JSON::Object;
...
HttpParamJSON RoomManager::leaveRoom(int room_id, int user_id) {
    Poco::JSON::Object::Ptr result = new Poco::JSON::Object;
...
HttpParamJSON RoomManager::deleteRoom(int room_id, int user_id) {
    Poco::JSON::Object::Ptr result = new Poco::JSON::Object;
...
HttpParam<> RoomManager::check_exit(Poco::JSON::Object::Ptr result,
